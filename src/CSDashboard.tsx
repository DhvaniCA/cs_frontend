...
const CSDashboard: React.FC = () => {
  ...
  // Admin → all levels
  // Student → only their own cs_level (+ "Others")
  const getAllowedLevels = useCallback((): Level[] => {
    ...
    if (!userLevel) return ["CSEET", "Executive", "Professional", "Others"]; // fallback until loaded

    // Always include "Others" as supplementary material
    const allowed: Level[] = userLevel === "Others"
      ? ["Others"]
      : [userLevel, "Others"];
  ...
  // ── Filter the full tree down to what the user may see ────
  const getVisibleTree = useCallback((): any => {
    const allowed = getAllowedLevels();
    const visible: any = {};
    for (const lvl of allowed) {
      if (tree[lvl]) visible[lvl] = tree[lvl];
    }
    return visible;
  }, [tree, getAllowedLevels]);
    ...
    <div className="cs-dashboard">
      ...
      <div className="cs-header">
        <div className="cs-header-inner">
          ...
          {/* Show user's level badge for students */}
      ...
      {!selected && (
        <div className="level-selector-section">
          ...
          {/* Contextual hint differs for admin vs student */}
          {isAdmin ? (
            <p className="level-selector-hint">
              Admin view — select any level to manage its content
            </p>
          ) : (
            <p className="level-selector-hint">
          ...
          {treeLoading ? (
            ...
            <div className="level-cards-grid">
              {(["CSEET", "Executive", "Professional", "Others"] as Level[]).map((lvl) => {
                // Hide levels not permitted for this user
                if (!allowedLevels.includes(lvl)) return null;

                const meta         = LEVEL_META[lvl];
                const subjectCount = visibleTree[lvl] ? Object.keys(visibleTree[lvl]).length : 0;
                const pdfCount     = countLevelPdfs(lvl);

                // Highlight the user's own level
                ...
                return (
                  ...
                  >
                    {isUserLevel && (
                      <span className="level-card-my-badge">My Level</span>
                    )}
                    <span className="level-card-icon">{meta.icon}</span>
                    <span className="level-card-name">{lvl}</span>
                    <span className="level-card-desc">{meta.desc}</span>
